...
function stripHTML (str){
  ...
  str = str.replace(/<(?:\/p|\/tr|\/table|\/div|\/[hH][1-6])>/g,'-\u0000\u0000--\u0000\u0000-');

  // convert linebreaks and div openings to linebreak markers
  str = str.replace(/<(br\s*\/*|div[^>]*)>/g,'-\u0000\u0000-');
  ...
  str = str.replace(/<a ([^>]*)>(.*?)<\/a[^>]*>/ig,function(match, params, content){
    ...
    var mailto = url.match(/(^mailto:|^tel:)/);

    return content.trim() + (url && !mailto ? ' (' + url + ')' : '');
  });
